...
- **Security posture (hard requirement):** key-only auth, per-device keys in hardware keystore, an SSH **forced-command broker** that whitelists tmux subcommands + scopes to an `agents:` session prefix, host-key pinning (TOFU via QR), per-device revocation, and push payloads that carry **only** `{session_id, state}` — never code. *Revised 2026-07-09:* public ingress to the **broker-only** endpoint is permitted and is the default UX (phone needs nothing but the app); a tailnet is an optional hardening layer, not a requirement. See §8.1.
...
- **Reachability** (*revised 2026-07-09 — the original "no public ingress" rule made the phone depend on a VPN app; product direction is scan-QR-and-go*). The QR carries whatever address the phone can dial, in priority order: the host's own public IP (cloud/VPS), a `--via` reverse tunnel through a box you own (laptop behind NAT; the relay forwards only ciphertext, SSH terminates on the laptop), or a tailnet IP as an optional fallback. What makes public exposure acceptable is that the listener is the **dedicated broker-only sshd**: pubkey-only, no passwords/PTY/forwarding, every key forced-commanded to the broker and scoped to one session — never the user's real sshd. The enroll listener runs only during pairing (single-use token, short TTL); hardening tracked: seal enrollment to the host key to close the on-path window.
